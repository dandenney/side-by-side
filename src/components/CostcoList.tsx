...
export function CostcoList() {
  ...
  return (
    <div className="bg-gradient-to-b from-blue-700 to-blue-600 relative min-h-[100dvh] h-full flex flex-col overflow-hidden">
      {/* List Items */}
      <div className="flex-1 overflow-y-auto overscroll-none px-4 pb-24">
        <div className="space-y-2 pt-4">
          <h1 className='opacity-60 text-center text-blue-900 uppercase font-bold'>Costco</h1>
            ...
            {items.map(item => (
              ...
              >
                <motion.div 
                  ...
                  layout
                >
                  <ShoppingBasket className={`w-5 h-5 transition-all ${item.checked ? 'text-red-600' : 'text-gray-400'}`} />
                </motion.div>
                ...
                {editingItem?.id === item.id ? (
                  <input
                    ...
                    className="flex-1 px-2 py-1 border rounded focus:outline-none focus:ring-2 focus:ring-blue-600"
                    autoFocus
                  ...
                  <motion.span 
                    layout
                      className={`flex-1 text-red-700 ${item.checked ? 'line-through text-gray-500' : ''}`}
                  >
                    {item.name}
                  </motion.span>
                )}
        ...
        <div className="grid grid-cols-3 gap-8 px-4 items-center justify-items-center">
          ...
          <div>
            <motion.button
              ...
              whileTap={{ y: 4 }}
              className="w-20 h-20 bg-gradient-to-b from-red-600 to-red-700 border-8 border-white -mt-12 text-white rounded-full shadow-[0_-4px_20px_rgba(0,0,0,0.15)] active:from-red-700 active:to-red-800 focus:outline-none focus:ring-2 focus:ring-red-800 flex items-center justify-center"
            >
          ...
          <div className='justify-self-start'>
            <motion.button
              ...
              className={`bg-gradient-to-b ease-out h-10 rounded-full flex items-center justify-center gap-2 relative rounded-full shadow-2xl shadow-inner shadow-black/10 transition-all w-10 ${hasCheckedItems
                ? 'from-red-600 to-red-700 opacity-80 focus:ring-2 focus:ring-red-600 cursor-pointer'
          ...
          <>
            ...
            >
              <form onSubmit={addItem} className="flex gap-2">
                ...
                <button
                  type="submit"
                  className="bg-gradient-to-b from-red-600 to-red-700 px-4 py-2 text-white rounded-lg active:from-red-700 active:to-red-800 focus:outline-none focus:ring-2 focus:ring-red-600"
                >
                  Add
                </button>
                <button
